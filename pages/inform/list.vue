...
<script>
  ...
  export default {
    data() {
      return {
        collectionList: [ db.collection('inform').field('user_id,category_id,title,content,excerpt,article_status,comment_status,last_comment_user_id,avatar,publish_date,last_modify_date,mode').getTemp(),db.collection('opendb-news-categories').field('name as text, _id').getTemp() ],
        query: '',
        where: '',
        orderby: dbOrderBy,
        orderByFieldName: "",
